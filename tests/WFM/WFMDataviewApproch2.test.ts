...
import { getRowNumberByCellValue, getRowNumberByEmpId, writeResultsToExcel, writeResultToExcel } from '@lib/Excel';
...
for (const empId in groupedData) {
    ...
    test(`@WFM Dataview Approch2 for ${empId}`, async ({ loginPage, wfmhomepage, wfmDatapage, }) => {
        ...
        await test.step('Open Maintenance Menu page', async () => {
            ...
            let i = 0;
            let row:number= await getRowNumberByEmpId(excelFilePath, sheetName, dataSet[index].EmpID);
