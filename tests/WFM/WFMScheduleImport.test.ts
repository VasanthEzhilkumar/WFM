...
const excelFileName = 'TimecardImport_Test.xlsx';
...
for (const empId in groupedData) {
    ...
    test(`@WFM Schedule Import for ${empId}`, async ({ loginPage, wfmhomepage, wfmtimecardpage, webActions }) => {
        ...
        await test.step('Open schedule Planner page', async () => {
            ...
            await wfmhomepage.clickImportExportData('H:/WFM/Data/emp_ImportData.xlsx');
            // await wfmhomepage.selectATemplateToImportData();
            // await wfmhomepage.clickOnOpenFile();
